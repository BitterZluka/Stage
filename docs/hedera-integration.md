...
| `USER_PERK_PAYMENT` | user wallet only | unique `PerkPurchase` + transaction reference | the browser never resubmits automatically after receiving a transaction reference | verify native token transfers or EVM HTS facade calldata through Mirror; reject reused/mismatched references | exact payer, token, treasury destination, amount, and `SUCCESS`; then create claim |
