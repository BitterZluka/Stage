...
| Auth | `POST /auth/session` | `{ challengeId, signature }` → `{ user: { id, accountIds }, expiresAt }` + httpOnly cookie | Public; one-time challenge | `LOGIN_CHALLENGE_INVALID`, `WALLET_ACCOUNT_NOT_FOUND`, `SIGNATURE_INVALID`, `SIGNATURE_VERIFICATION_UNAVAILABLE` | Mirror account-key/EVM address read | sync |
