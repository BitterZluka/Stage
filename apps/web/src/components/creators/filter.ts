...
export function matchesCreatorFilters(
  ...
  if (query) {
    const haystack =
      `${creator.displayName} ${creator.username} ${creator.category} ${creator.bio} ${creator.tokenSymbol}`.toLowerCase();
  ...
  if (filters.activity.length > 0) {
    const matchesActivity = filters.activity.some((activity) => {
      if (activity === "active-challenges")
        return creator.activeChallengesCount > 0;
  ...
  if (
    filters.communitySize.length > 0 &&
    !filters.communitySize.includes(communitySizeOf(creator))
  ) {
    return false;
  }
